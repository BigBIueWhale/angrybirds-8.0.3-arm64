...
bit‑reproducible binary (<code>45d17484…</code>), so it can be regenerated rather than taken on trust.</em></p>
...
> same order, same sizes, same call sites, same sha256 — at **two depths**: **7793** records
> through the constructors, and **8290** through `nativeInit`, which drives JNI passthrough to
> the JVM boundary and so reaches past boot into the lifecycle
> (`port/validation/alloc_trace_compare.sh`). The older *"7792 of 7793 match"* figure was
> measured before the allocator size‑class fix; that one divergent allocation is now gone.
...
| **What it can actually do** | **Measured on Android 16, the app is granted exactly one real permission: `WAKE_LOCK`** (normal-level, auto-granted, keeps the screen awake during play). Its own self-declared `…X2D_MESSAGE` is also granted and confers nothing. **Ten** permissions are mangled to unresolvable strings — `INTERNET`, `ACCESS_NETWORK_STATE`, `GET_ACCOUNTS`, `vending.BILLING`, `vending.INSTALL_REFERRER`, the three `c2dm` push permissions, the Play install-referrer service, and the app's own `C2D_MESSAGE` — and none of them appears in any granted list. `WRITE_EXTERNAL_STORAGE` **is** still declared and **is** `dangerous`, but it is `granted=false` and the game never requests it: every one of the 48 files it writes is app-private (`OPEN_FINDINGS.md` R5, R7). |
| **Install‑time integrity** | four things a user would otherwise discover the hard way are checked mechanically: the APK still declares **`com.rovio.angrybirds` 8.0.3** (a renamed package installs as a *second app*, not an update); **only `arm64-v8a`** native libs ship (a surviving 32‑bit dir could make Android load Rovio's original ARM32 engine, which the phone cannot execute); it is signed by the repo's committed key **`d56d5b2e…`** (a different signer forces an uninstall and loses saves); and **every SHA‑256 printed in these docs is re‑derived from the artifact** — the audio variant's was wrong until 2026‑07‑28 |
