...
| **What it can actually do** | after conversion the manifest grants exactly **one** real permission: `WAKE_LOCK` (normal‑level, auto‑granted, keeps the screen awake during play) plus `com.sec.android.airview.HOVER`, a Samsung UI hint. `INTERNET`, `GET_ACCOUNTS` and `com.android.vending.BILLING` survive only as `XNTERNET`, `XET_ACCOUNTS`, `XILLING` — strings Android cannot resolve to a permission, so it grants none of them. **No dangerous permissions, so installing prompts for nothing.** Verify with `strings` on the manifest, or read the before/after in `port/OPEN_FINDINGS.md` |
