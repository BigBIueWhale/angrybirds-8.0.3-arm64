...
The case that the x86 rig predicts arm64 behaviour is reasonable but not airtight — identical shim source, and Unicorn runs the ARM32 guest faithfully on either host (125/125 C++ constructors execute clean on both, and the guest allocation sequence matches for at least the first 4913 requests). The guest heap has been claimed both ways in this file and neither wording survived contact with a re-measurement, so here is the current state with its history. An early version claimed the heap was *bit-identical* across host architectures. That was corrected to “differs by about 64 KB after startup, deterministically per host” — which was true when measured. **Re-measured 2026-07-28, that gap is no longer reproducible: x86 and AArch64 both report `605096` bytes in use after the 125 constructors**, and `605096` again when rebuilt without `-DRTLD_DEFAULT=0`, ruling out the flag difference between the two suites. The old arm64 figure of `539536` cannot be obtained any more; the one variable not eliminated is how the AArch64 binary is produced (the old number came from a build made *inside* an emulated arm64 container, the new ones from a cross toolchain). Treat the heap as **measured identical across architectures today**, while noting the figure has moved before and is worth re-measuring rather than quoting.

The per-allocation comparison is stronger than it used to be as well: `alloc_trace_compare.sh` now reports **all 7793 ctor allocation records identical** (sha256 `a4407f9f28cae7f1…`) and **all 8290 `nativeInit` records identical** (`3f7c7bcc30409d06…`) between the two hosts — the earlier notes said “at least the first 4913 requests”.

None of that makes the x86 rig a substitute for the phone: both architectures execute the engine correctly, but “it passed on x86” still does not by itself establish “it will behave identically on arm64”. The arm64 ABI itself is validated separately under qemu‑user: the real engine loads and all 125 C++ static constructors execute on AArch64. But that is emulation of the *ABI*, not the game — no Android, no ART, no JNI, no GL, no frames.
...
| **Modern Android** | targetSdk 26 installs; `libm` linked; ELF **16 KB‑page aligned** — the requirement for 16 KB‑page devices, asserted on every build; loading on one is inferred, not observed (no such image exists on the build host); JIT runs under W^X |
| **No phone‑home** | four independent layers, **all four verified at runtime on Android 16** (and one category they cannot cover — an `ACTION_VIEW`/`market://` intent is an IPC to Android, not a socket — measured at **zero** across a full playthrough, `OPEN_FINDINGS` R13). The app's own pid performs **zero** name resolution or socket work (measured by pid — the DNS failures in a full `logcat` belong to the emulator's NetworkMonitor); the shipped shim **imports no socket symbols at all**; and the FCM kill‑switch is proven *differentially* — the same build without it attempts token registration, this one attempts it zero times |
