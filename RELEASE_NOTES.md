...
The case that the x86 rig predicts arm64 behaviour is genuinely strong — identical shim source, Unicorn emulates the ARM32 guest deterministically regardless of host architecture, and the guest heap was verified bit‑identical between x86 and arm64 runs. The arm64 ABI itself is validated separately under qemu‑user: the real engine loads and all 125 C++ static constructors execute on AArch64. But that is emulation of the *ABI*, not the game — no Android, no ART, no JNI, no GL, no frames.
