...
ABLOG="$OUT/emu_fatal_abshim.txt"; : >"$ABLOG"
echo "== boot ==" | tee -a "$LOG"
...
adb exec-out screencap -p > "$OUT/emu_fatal_screen.png" 2>/dev/null
echo "screenshot: $(wc -c < "$OUT/emu_fatal_screen.png" 2>/dev/null)B" | tee -a "$LOG"
echo DONE | tee -a "$LOG"
