...
| `INSTALL_FAILED_UPDATE_INCOMPATIBLE` | A previous install signed with a different key | Confirm which key your APK carries before uninstalling anything: `apksigner verify --print-certs <apk> \| grep 'SHA-256 digest'` should print `d56d5b2eabc7953917e5d85605842548b0dfafe183b0aa1b75174b5fffd5deb4`, the repo's committed `port/debug.ks`. If it does, every build from this repo update-installs over every other and you can keep your saves; if it differs, that APK came from a checkout **missing the keystore** — the build scripts silently mint a random key in that case — and you would have to `adb uninstall com.rovio.angrybirds` first, losing saves. `verify_claims.sh` checks this automatically |
